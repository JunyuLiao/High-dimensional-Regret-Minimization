...
highdim_output* interactive_highdim(point_set_t* skyline, int size, int d_prime, int d_hat, int d_hat_2, point_t* u, int rounds, int s, double epsilon, int maxRound, double& Qcount, double& Csize, int cmp_option, int stop_option, int prune_option, int dom_option, int& num_questions){
	...
	for (int i=0;i<d/d_hat;++i){ // still need to modify to handle corner cases where d is not divisible by d_hat
        ...
        double maxValue = 0;
        int maxIdx = 0;
        // create a subset of u with d_prime dimensions
		...
		if (id!=-1){
			for (int j=0;j<d_hat;++j){
				selected_dimensions.insert(i*d_hat+j);
			}
		}
		release_point_set(S, false);
        release_point_set(D, true);
	}
    // if d/d_hat is not an integer, we need to handle the corner case
    if (d % d_hat != 0){
        for (int j= (d/d_hat)*d_hat;j<d;++j){
            selected_dimensions.insert(j);
        }
    }   
	// phase 2: narrow down the at most d_hat*d' dimensions further
    // printf("Phase 2\n"); // 222
	// apply the Generalized_binary-splitting_algorithm
	bool answer_q = false; // check if the user is willing to answer more questions
    ...
    int d_target = d_prime;
	while (d_left > 0 && answer_q && final_dimensions.size() < d_prime){
		...
		if (d_left <= 2*d_target-2){
			for (int i=0;i<d_left;++i){
				...
				answer_q = true; //user may not want to answer more questions
            ...
            if (group_size == 1){
                ...
                for (int j = 0; j < size; ++j) {
                    int idx = dis(gen);
                    S->points[j] = alloc_point(group_size);
                    S->points[j]->id = D->points[idx]->id;
                    for (int p = 0; p < group_size; ++p) {
                        S->points[j]->coord[p] = D->points[idx]->coord[p];
                    }
			...
			if (id == -1){
                ...
                if (group_size == 1){
                    ...
                    while (left < right){
                        ...
                        for (int j = 0; j < size; ++j) {
                            int idx = dis(gen);
                            S->points[j] = alloc_point(mid - left + 1);
                            S->points[j]->id = D->points[idx]->id;
                            for (int p = 0; p < mid-left+1; ++p) {
                                S->points[j]->coord[p] = D->points[idx]->coord[p];
                            }
    ...
    if (final_dimensions.size() == d_prime){
    ...
    // 555
    // printf("the final dimensions are: ");
    // for (int j = 0; j < set_final_dimensions.size(); ++j) {
    //     printf("%d ", *next(set_final_dimensions.begin(), j));
    // }
    // printf("\n");
	...
	point_set_t* D = alloc_point_set(n);
	for (int j=0;j<n;++j){
		D->points[j] = alloc_point(final_d);
		D->points[j]->id = skyline->points[j]->id;
		for (int p=0;p<final_d;++p){
			D->points[j]->coord[p] = skyline->points[j]->coord[*next(set_final_dimensions.begin(), p)];
		}
	}

    point_set_t* skyline_D = skyline_point(D);
    //print the number of points in skyline_D
    // printf("num of skyline points in D: %d\n", skyline_D->numberOfPoints); // 666
	...
	if (final_d == d_prime){
        ...
        S_output->points[0] = max_utility(D, u_final, s, epsilon, maxRound, Qcount, Csize, cmp_option, stop_option, prune_option, dom_option);
    }
	else {
        S_output = attribute_subset(skyline, S_output, final_d, d_prime, d_hat_2, rounds, set_final_dimensions);
        double mrr_value = evaluateLP(skyline, S_output, 0, d_prime, set_final_dimensions, 30);
        // for testing purpose, run the loop 10 times and show the mean and std
        std::vector<double> mrr_values;
        std::vector<double> S_output_sizes;
        point_set_t* S_output_temp = nullptr;
        for (int i = 0; i < 10; ++i){
            S_output_temp = nullptr;
            S_output_temp = attribute_subset(skyline, S_output_temp, final_d, d_prime, d_hat_2, rounds, set_final_dimensions);
            double mrr_value_temp = evaluateLP(skyline, S_output_temp, 0, d_prime, set_final_dimensions, 30);
            mrr_values.push_back(mrr_value_temp);
            S_output_sizes.push_back(S_output_temp->numberOfPoints);
            release_point_set(S_output_temp, false);
        }
        double mean_mrr = 0;
        double mean_size = 0;
        double std_mrr = 0;
        double std_size = 0;
        for (int i = 0; i < mrr_values.size(); ++i){
            mean_mrr += mrr_values[i];
            mean_size += S_output_sizes[i];
        }
        mean_mrr /= mrr_values.size();
        mean_size /= S_output_sizes.size();
        for (int i = 0; i < mrr_values.size(); ++i){
            std_mrr += (mrr_values[i] - mean_mrr) * (mrr_values[i] - mean_mrr);
            std_size += (S_output_sizes[i] - mean_size) * (S_output_sizes[i] - mean_size);
        }
        std_mrr = sqrt(std_mrr / mrr_values.size());
        std_size = sqrt(std_size / S_output_sizes.size());
        // print the mean and std
        printf("mean mrr: %f, std mrr: %f\n", mean_mrr, std_mrr);
        printf("mean size: %f, std size: %f\n", mean_size, std_size);
	}
